...
const ProjectCards = ({project_infos}) => {
  return (
    project_infos.card_grid.map((column, i) => (
        <div key={i} className='grid grid-cols-3 grid-rows-13 w-full gap-4 md:gap-8'>
          {column.cards.map((card, i) => (
            <div key={i} className={`relative col-start-1 col-end-4 row-start-${card.row_start} row-end-${card.row_end} border border-[#00171F] dark:border-[#F4E285] transition-all ease-in-out group`}>
